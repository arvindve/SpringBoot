...
public class StatusUpdateController {
	...
	ModelAndView editStatus(ModelAndView modeAndView, @RequestParam(name="id") Long id) {
		
		StatusUpdate statusUpdate = statusUpdateService.get(id);
		modeAndView.getModel().put("statusUpdate", statusUpdate);
		
		modeAndView.setViewName("app.editStatus");
		return modeAndView;
		
	}
	
	@RequestMapping(value="/editstatus", method=RequestMethod.POST)
	ModelAndView updateStatus(ModelAndView modeAndView, @Valid StatusUpdate statusUpdate , BindingResult result){
		
		if(!result.hasErrors()){
			statusUpdateService.save(statusUpdate);
			modeAndView.setViewName("redirect:/viewstatus");
		}
		return modeAndView;
